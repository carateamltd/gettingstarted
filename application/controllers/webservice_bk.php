...
class webservice extends MY_Controller 
{
    ...
    function send_list_mailchimp($apikey,$email,$listid)
    {
        ...
        $result = $MailChimp->call('lists/subscribe', array(
                        ...
                        'merge_vars'        => array( 'FNAME' => 'MailingList', 'LNAME' => 'Test', 'COMPANY' => 'Carateam ltd', 'STATE' => 'France'),
                        'double_optin'      => false,
       ...
       // $mconfig['apikey'] = $apikey;
        //$this->load->library('MCAPI', $mconfig);
       ...
       /* $emailAddress = $email;#'pradip.carateamltd@gmail.com';
    ...
    **/
	...
	function send_order_mail_admin_details($tEmail,$iOrderId)
	{
        ...
        $this->email->to('carateamltd@gmail.com'); 
...
header('Access-Control-Allow-Origin: *');
    ...
    **/
    function google_cloud_printer_upload()
    {
        ...
        if($gcp->loginToGoogle(/*Add email address*/ , /*Add password email*/)) {
    ...
    {
		// api details
	   /*$api_details = $this->webservice_model->get_api_details_application($payment_details['iApplicationId']);
	   $config = array(
            'Sandbox' => ($api_details[0]['eStatus'] == 'Enabled' ? TRUE : false),
            'APIUsername' => $api_details[0]['tAPIUserId'],
            'APIPassword' => $api_details[0]['tAPIPassword'],
            'APISignature' => $api_details[0]['tAPISignature'],
            'APISubject' => '',             
            'APIVersion' => '98.0'      
        );
        //print_r($config);exit;
	    ...
	    $this->load->library('paypal/Paypal_pro_client', $config, $paypal);*/
    ...
    **/
	...
	function easyapps_item_get()
	{
		...
		if($Data['iApplicationId'] != ''){
			...
			array_walk($get_item_category, function($val,$key) use(&$get_item_category){
                $temp_size = $this->webservice_model->get_item_size_option($val);
                $temp_options = $this->webservice_model->get_item_option_option($val);
                $size = array();
                $option = array();
                foreach($temp_size as $tmp){
                    $size[] = $tmp['vSizeName'];
                }
                foreach($temp_options as $tmp){
                    $option[] = $tmp['vOptName'];
                }
                $get_item_category[$key]['all_sizes']=implode(',',$size);
                $get_item_category[$key]['all_options']=implode(',',$option);
            });
			...
			if(count($get_item_category) >0){
				foreach($get_item_category as $val){
					...
					$category[] = array(
						...
						'eStatus'=>$val['eStatus'],
						'vCurrency' => $currency,
						'all_sizes'=>$val['all_sizes'],
                        'all_options'=>$val['all_options'],
                        'vName' => $menuTitle['vName'] 
					);
				}
				/** category **/
...
header('Access-Control-Allow-Origin: *');
	...
	function easyapps_total_show_tabs_details()
	{
		...
		if($Data['iApplicationId'] != '')
		{
			...
			if(count($features) > 0)
			{
				/** total show tabs **/
				foreach($features as $val){
					$features_details[] = array('iAppTabId'=>$val['iAppTabId'],
						'iApplicationId'=>$val['iApplicationId'],
	...
	function easyapps_menuofthe_day()
	{
		...
		if($Data['iApplicationId'] != '')
		{
			...
			*/
			$vCurrency = $this->webservice_model->get_currency_for_order_details($Data['iApplicationId']);
			if($vCurrency['vCurrency'] == 'EUR'){$vCurrency='&euro;';}else if($vCurrency['vCurrency'] == 'USD'){$vCurrency='$';}else{$vCurrency=$vCurrency['vCurrency'];}
           
            if($menu_day){
                /** total show tabs **/
				foreach($menu_day as $val){
					$Menu_day_details[] = array('iItemId'=>$val['iItemId'],
						'iAppTabId'=>$val['iAppTabId'],
						'iApplicationId'=>$val['iApplicationId'],
						'iMenuId'=>$val['iMenuId'],
						'vItemName'=>$val['vItemName'],
				...
				/** Details **/
				$Data['Menu_day'] = $Menu_day_details;
				$Data['status'] = "Success";
            }else{
                $Data['status'] = 'Fail';
            }
	...
	**/
	function tousles_easyapps_bridge()
	{
        ...
        $this->email->initialize(array(
            'protocol' => 'smtp',
            'smtp_host' => 'ssl://smtp.gmail.com',
            'smtp_user' => 'caratealtd@gmail.com',
            'smtp_pass' => 'nopass',
            'smtp_port' => 465,
            'mailtype'  => 'html',
            'charset'   => 'iso-8859-1',
        ...
        $body = 'Hello,<br /><br />Please find login credential for your EasyApps Bronze Account as under<br /><br /><b>Username : </b>'.$data->vEmail.'<br /><b>Password : </b>'.$pass.'<br /><br /><br /> Thank You, <br />Touslesrestos.fr';

        $this->email->from('info@touslesrestos.fr', 'Touslesrestos.fr');
        $this->email->to($data->vEmail);
        $this->email->subject('EasyApps Free Bronze Account Details');
		...
		foreach($productcategory as $val)
		{
			...
			foreach($productoption as $sizeval){
				$optiondetails[] = array(
					...
					'fOptionPrice'=> $sizeval['fOptionPrice'],
		...
		foreach($maincategory as $val)
		{
			...
			foreach($subcategory as $subval){
				$subcat[] = array(
					...
					'vImage' => $this->data['base_url'].'uploads/cataloguesub/'.$val['iCatalogueSubId'].'/'.$val['vImage'],
					'leaf' => true,
				);
			}
