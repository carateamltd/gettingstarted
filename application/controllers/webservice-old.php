...
header('Access-Control-Allow-Origin: *');
	...
	*/
	function easyapps_order_customer_details()
	{
		...
		if(count($Data) >0){
			...
			if($insert){
				...
				$result['emailClient'] = send_order_detail_email($Data['tEmail'],$appOwnerEmail,$emailData);//$this->send_order_client_mail($Data['tEmail']);
 				$result['emailAdmin'] = send_order_detail_email($appOwnerEmail,$Data['tEmail'],$emailData);//$this->send_order_admin_mail($Data['tEmail']);
	...
	function easyapps_catalogue_customer_details()
	{
 	...
 	public function send_order_detail_email($toEmail,$fromEmail,$data)
 	{
		/*		client reservation		*/
		$ci = get_instance();
		$ci->load->library('email');
	...
	//	$html .= '<p>Votre Réservation a été envoyée au centre de service suivant:</p>';
		...
		$ci->email->initialize($config);
		$ci->email->from("raghavendra.soni@aplitetech.com", 'easyapps');
		$ci->email->to($vEmail);
		$this->email->reply_to("raghavendra.soni@aplitetech.com", 'Explendid Videos');
		$ci->email->subject('Reservation');
		$ci->email->message($html);
