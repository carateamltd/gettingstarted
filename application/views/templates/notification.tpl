				...
				<form class="form-horizontal" id="form_notification"   method="post" action="{$data.base_url}notification/sendPushMessage">
                        ...
                        {/foreach}</label>
                                    <div class="controls">
									   ...
									   <option value="{$data['user_applications'][i]}">-- {foreach from=$lang item=val}
                             {if $val.rLabelName == 'Select iApplicationIdon'}
                                {$val.rField}
                             {/if}
                        {/foreach} --</option>
									    {section name=i loop=$data['tabname']}
									   		<option value="{$data['tabname'][i]['iFeatureId']}">
                                                {foreach from=$lang item=val}{if $val.rLabelName == $data['tabname'][i]['vTitle']} {$val.rField} {/if} {/foreach}
                                            </option>
										{/section} 
									   </select>
                                    </div>
