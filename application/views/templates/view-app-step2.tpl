              ...
              <div class="div_inner">
                ...
                <div id="tbl_msg"></div>
                                  <tr class="nodrop">
                                    ...
                                    <th style="display: none;">{foreach from=$lang item=val}
                                    ...
                                    <th style="text-align:center;">{foreach from=$lang item=val}
                                     ...
                                     {/if}
                                     {/foreach}</th>
                                    <th style="text-align:center;" colspan="2">{foreach from=$lang item=val}
                                  ...
                                  <tr class="row1a" id="recordsArray_{$data.selected_feature_details[i].iAppTabId}">
                                    <td  style="text-align:center;"class="handle_tr icon"><!--{*SEQ*}-->
                                      <img width="16" height="16" alt="move" src="{$data.base_image}icon_move.png" /></td>
                                      <!--
                                      Name:- Hem
                                      Date:- 16-jun
                              ...
                              <img width="25" height="25" src="{$data.base_upload}tab_icon/{$data.selected_feature_details[i].iIconcolorId}/{$data.selected_feature_details[i].vImage}" /></td>
                                    <td width="12%" style="text-align:center;"><!-- {$data.selected_feature_details[i].eActive} -->
                                    {foreach from=$lang item=val}{if $val.rLabelName == $data.selected_feature_details[i].eActive} {$val.rField} {/if} {/foreach}</td>
                                    <td width="30%" style="text-align:center;">
                                        
                                      {foreach from=$lang item=val}{if $val.rLabelName == $data.selected_feature_details[i].vTitle} {$val.rField} {/if} {/foreach}
                                        <!--  -->
                                        </td>
                                    <td width="30%" style="text-align:center;"><!-- {$data.selected_feature_details[i].default_vTitle} Tab -->
                                    {foreach from=$lang item=val}{if $val.rLabelName == $data.selected_feature_details[i].default_vTitle} {$val.rField}{/if} {/foreach}
                                    </td>
                                    <td style="text-align:center;"><a data-toggle="modal" href="#" onclick="edit_custom_tab('{$data.selected_feature_details[i].iAppTabId}','{$data['step']}');">
                                    <i class="icon-pencil helper-font-24" title="Edit"></i></a></td>
                                    <td style="text-align:center;"><a data-toggle="modal" href="#" onClick=delete_tab({$data.selected_feature_details[i].iAppTabId},{$data.iApplicationId});>
      ...
      <tr>
        ...
        <td valign="top">
          ...
          <select class="indst" id="icon_iFeatureId" name="data[iFeatureId]">
            <option value="">{foreach from=$lang item=val}{if $val.rLabelName == 'Select Tab'}{$val.rField}{/if}{/foreach}</option>
        ...
        <td valign="top"><input type="hidden" value="No"  name="data[eActive]"><input class="indst" type="checkbox" maxlength="12" value="Yes" size="30"  name="data[eActive]" checked>{foreach from=$lang item=val}{if $val.rLabelName == 'Active'}{$val.rField}{/if}{/foreach} Tab</td>
      </tr>
...
<div class="modal fade" id="myModalDelete" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">{foreach from=$lang item=val}{if $val.rLabelName == 'Close'}{$val.rField}{/if}{/foreach}</button>
        <a href="" id="mylink"><button type="button" class="btn btn-primary">{foreach from=$lang item=val}{if $val.rLabelName == 'Delete'}{$val.rField}{/if}{/foreach}</button></a>
      </div>
    </div>
  </div>
</div>
