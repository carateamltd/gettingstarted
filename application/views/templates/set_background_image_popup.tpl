...
<div id="myModal_set_backimg" class="modal hide fade modalsetbg" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-header">
    ...
    <h3 id="myModalLabel">{foreach from=$lang item=val}
                    {if $val.rLabelName == 'Set a Background Image'}
                       {$val.rField}
                     {/if}
                     {/foreach}</h3>
  </div>
  ...
  <div class="modal-body">
    ...
    <div class="background_tab">
		<ul>
			<li ><a href="#background_tab_yi">{foreach from=$lang item=val}
                    {if $val.rLabelName == 'Your Image'}
                       {$val.rField}
                     {/if}
                     {/foreach} </a></li>
			<li ><a href="#background_tab_pi">{foreach from=$lang item=val}
                    {if $val.rLabelName == 'Preset Image'}
                       {$val.rField}
                     {/if}
                     {/foreach} </a></li>
		</ul>
		<form name="frm_add_back_img" id="frm_add_back_img" method="POST" action="{$data.base_url}app/set_background_image" enctype="multipart/form-data">
		<div class="uploadimage">
			<input type="file" name="vImage" id="file" onchange="CheckValidFile(this.value,'file');" style="width:220px;">
			  <button type="button" class="btn btn-inverse" id="upld_bcimg" >{foreach from=$lang item=val}
                    {if $val.rLabelName == 'Upload Photo'}
                       {$val.rField}
                     {/if}
                     {/foreach}
              </button>
		...
		<div class="background_box">
				{section name = i loop = $data.your_tabbackground}				
					<div class="boxwrapimg">
						...
						<div class="boxinfo">
							<h3 id="bac_img_{$data.your_tabbackground[i].iBackgroundId}" class="imagenamewrap" style="width:80%;">{$data.your_tabbackground[i].vImage}</h3>
							<!--p>640 x 860</p>
							<p>425.21 KB</p-->
							<span style="padding-top:5%;float:left;"><a href="{$data.base_upload}background_image/{$data.your_tabbackground[i].iBackgroundId}/{$data.your_tabbackground[i].vImage}" target="_blank"><img src="{$data.base_image}preview-icon.png" width="20" height="18" alt="" /></a></span>					</div>
						</div>
				{/section}
					</div>
  ...
  <div class="modal-footer">
    <span id="sel_back_img_name" style="color:black;">{foreach from=$lang item=val}
                    {if $val.rLabelName == 'No image selected'}
                       {$val.rField}
                     {/if}
                     {/foreach} </span>
    <button class="btn" data-dismiss="modal" aria-hidden="true">{foreach from=$lang item=val}
                    {if $val.rLabelName == 'Close'}
                       {$val.rField}
                     {/if}
                     {/foreach}</button>
    <button type="button" class="btn btn-primary" id="save_bcimg" style="display:none;">{foreach from=$lang item=val}
                    {if $val.rLabelName == 'Save Choice'}
                       {$val.rField}
                     {/if}
                     {/foreach}</button>
  </div>
