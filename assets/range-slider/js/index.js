...
$(function() {
	...
	function updateOpeningHours2bkp() {
		if ($('#schedule').length) {
	...
	function updateOpeningHours() {
		for (d=1; d<=7; d++) {
			document.getElementById('sday'+d).value=rangeTimes[d].length<1?'Closed':rangeTimes[d][0];
			document.getElementById('eday'+d).value=rangeTimes[d].length<1?'Closed':rangeTimes[d][1];
		}
	}
	
	function updateOpeningHours1(){
		for (d=1; d<=7; d++) {
			document.getElementById('s1day'+d).value=rangeTimes[d].length<1?'Closed':rangeTimes[d][0];
			document.getElementById('e1day'+d).value=rangeTimes[d].length<1?'Closed':rangeTimes[d][1];
		}
	}
