...
class DataService
{
     ...
     * @return <array>
     */
    public function getMessages($certificateId, $statusId, $limit,$token="") {
	if($token != ''){
		$sql = "SELECT DeviceToken, MessageId, Message, Badge, Sound FROM MessageQueue MQ, Devices D WHERE D.DeviceId = MQ.DeviceId AND D.DeviceToken ='".$token."' AND CertificateId = %d  AND MQ.Status != '2' LIMIT %d";
        $sql = sprintf($sql, (int) $certificateId, (int)$limit);
	}else{
     ...
     * @return <array>
     */
    public function getTokenMessages($certificateId, $token, $limit) {
